...
if (isset($_SESSION['authenticated'])) {
    if (isset($_POST['btn_placeOrder'])) {
        ...
        if ($name == "" || $email == "" || $phone == "" || $zipcode == "" || $address == "" || $payment_mode == "") {
    ...
    $receipt_filename = time().'.'.$image_extension;
        // end, not sure if dito ba ito

        $tracking_no = "cupcake" . rand(1111, 9999) . substr($phone, 2);

        $insert_query = "INSERT INTO orders(tracking_no, user_id, name, email,	phone, address,	zipcode, total_price, shippingFee, payment_mode,deliveryDate, img_receipt) VALUES ('$tracking_no','$userId','$name','$email','$phone','$address','$zipcode','$totalPrice','$shippingFee','$payment_mode', '$deliveryDate','$receipt_filename')";
